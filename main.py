...
def on_button_click():
    ...
    print(f"Downloading data for {ticker1} and {ticker2} from {start} to {end} with indicators: {indicators}")
    df1, df2 = load_data(ticker1, ticker2, start, end)
    print("Data downloaded successfully. Now plotting...")
    p1 = plot_data(df1, indicators)
    p2 = plot_data(df2, indicators, sync_axis=p1.x_range)
    print("Plots generated. Now updating the document...")
    curdoc().clear()
    curdoc().add_root(layout)
    curdoc().add_root(row(p1, p2))
    print("Document updated. The graphs should be visible now.")
